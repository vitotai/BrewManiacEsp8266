...
After version 0.5.2, an option name "Auto Resume". If it is enabled, the controller will try resuming automation process after 15 seconds from **power on** without no user intervention. This feature is designed to handle power shortage in the long brew day. The automatic resumption is available from the starting of **Delay Start**, so that if power shortage happens during the delay start, the automation process will be resumed. 