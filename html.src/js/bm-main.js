var BM = {
    ...
    sendButton: function(k, l, fh) {
        var btncode = { up: 1, down: 2, start: 4, enter: 8 };
        var code = 0;
        $.each(k, function(i, btn) {
            code += btncode[btn];
        });
        code = code + ((l) ? 16 : 0);
        //console.log("sendButton:"+code);

        $.ajax({
            url: this.button_url + "?code=" + code,
            type: "GET",
            success: function() {
                fh();
            },
        ...
        /* ws
        	this.ws.send('{"btn":'+ code+'}');
        	fh();
        */
    },
    /*getsetting:function(){
    	var o=this;
    	$.ajax({url:this.setting_url + "?time="+((new Date()).getTime()/1000),
    			type: "GET",
    			dataType : "json",
        		success: function( json ) {
    				if(json.code ==0){
    					o.proc_settings(json.data);
    				}
       			 },
        		error: function( xhr, status, errorThrown ) {
        			console.log(errorThrown);
    ...
    /*getauto:function(){
    	var b=this;
    	$.ajax({url:this.automation_url,
    			type: "GET",
    			dataType : "json",
        		success: function( json ) {
    				if(json.code ==0){
    					b.procRecipe(json.data);
    				}
       			 },
        		error: function( xhr, status, errorThrown ) {
        			console.log(errorThrown);
    ...
    setupEvent: function() {
        ...
        var es = new EventSource(BM.status_url);
        ...
        es.addEventListener('setting', function(e) {
            //console.log("setting:", e.data);
            b.proc_settings(JSON.parse(e.data));
        }, false);
        es.addEventListener('auto', function(e) {
            //console.log("auto:", e.data);
            b.procRecipe(JSON.parse(e.data));
        }, false);
        es.addEventListener('netcfg', function(e) {
            //console.log("netcfg:", e.data);
            b.piggy.netcfg = e.data;
            if (typeof b.piggyCB["netcfg"] != "undefined")
                b.piggyCB.netcfg(e.data);
        }, false);

        es.addEventListener('timesync', function(e) {
            //console.log("time sync:"+ e.data);
            var sync = JSON.parse(e.data);
